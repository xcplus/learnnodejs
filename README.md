...
3. session 运行流程
填写账号和密码校验身份，校验通过后创建session信息，然后将session_id的值通过响应头返回
给浏览器，有了cookie, 下次发送请求时会自动携带cookie， 服务器通过cookie中的session_id值确定用户身份
...
1. 存储的位置
  ...
  - session: 服务器端
2. 安全性
  - cookie 是以明文的方式存放在客户端，安全性相较低
  - session 存放在服务器端，安全性较好
3. 网络传输
  - cookie 设置内容过多会增大报文体积，影响传输效率
  - session 数据存储在服务器，只是通过cookie传递id, 所以不影响传输效率
4. 存储限制
  - 浏览器限制单个cookie保存的数据不能超过4k，且单个域名下的存储数量也有限制
  - session存储在数据库中，所以没有限制

### token
1. token 是服务端生成并返回给http客户端的一串加密字符串，token中保存着用户信息
2. token 的作用： 实现会话控制，可以识别用户的身份，主要用于移动端app
3. token 的工作流程：填写密码和账号校验身份，校验通过后响应token，token一般是在响应体中返回给客户端的
4. token 的特点
  - 服务端压力更小，数据存储在客户端
  - 相对更安全，数据加密，可以避免CSRF(跨站请求伪造)
  - 扩展性更强, 服务间可以共享，增加服务节点更简单
5. JWT (JSON Web Token) 是目前最流行的跨域认证解决方案，可用于基于token的身份验证
JWT使token的生成与校验更规范,可以使用jsonwebtoken 包操作token